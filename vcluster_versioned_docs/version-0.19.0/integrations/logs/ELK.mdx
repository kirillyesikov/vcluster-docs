...
<<<<<<<< HEAD:vcluster_versioned_docs/version-0.19.0/integrations/logs/ELK and fluentd.mdx
...
========
## Add the Helm repo and install a single-node ELK stack (make sure you have valid values.yaml's for all ELK components).You can download sample values files from here : [elastic_values.yaml](https://github.com/kirillyesikov/vcluster-docs/blob/main/vcluster_versioned_docs/version-0.19.0/integrations/elastic_values.yaml), [logstash_values.yaml](https://github.com/kirillyesikov/vcluster-docs/blob/main/vcluster_versioned_docs/version-0.19.0/integrations/logstash_values.yaml) [kibana_values.yaml](https://github.com/kirillyesikov/vcluster-docs/blob/main/vcluster_versioned_docs/version-0.19.0/integrations/kibana_values.yaml).
>>>>>>>> 01dcd924f1909b34c45b0ab68676a9c4100d58f2:vcluster_versioned_docs/version-0.19.0/integrations/metrics/elastic-stack.mdx
```
...
<<<<<<<< HEAD:vcluster_versioned_docs/version-0.19.0/integrations/logs/ELK and fluentd.mdx
### Install fluentd imperatively using  the daemonset , you can download the deamonset from [github](https://github.com/fluent/fluentd-kubernetes-daemonset/blob/master/fluentd-daemonset-elasticsearch-rbac.yaml):
...
1. Check for available indices and `port-forward` the elasticsearch-master service `kubectl port-forward -n logging svc/elasticsearch-master 9200` and visit the [http://localhost:9200/\_cat/indices](http://localhost:9200/_cat/indices),
    ...
    ```
 1. Next `port-forward` the kibana dashboard on its default port `kubectl port-forward -n monitor svc/elk-kibana-kibana 5601` and navigate to [http://localhost:5601/app/management](http://localhost:5601/app/management) or
